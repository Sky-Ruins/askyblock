...
public class IslandGuard implements Listener {
    ...
    public void onHorseInventoryClick(InventoryClickEvent event) {
        ...
        if (event.getInventory().getHolder() instanceof Animals) {
            ...
            Util.sendMessage(event.getWhoClicked(),
                    ChatColor.RED + plugin.myLocale(event.getWhoClicked().getUniqueId()).islandProtected);
    ...
    public void onVehicleMove(final VehicleMoveEvent e) {
        ...
        if (islandTo != null && (islandTo.getOwner() != null || islandTo.isSpawn())) {
            ...
            if (islandTo.isLocked() || plugin.getPlayers().isBanned(islandTo.getOwner(), player.getUniqueId())) {
                if (!islandTo.getMembers().contains(player.getUniqueId()) && !player.isOp()
                    ...
                    if (minMin < 1) {
                        Vector v = e.getVehicle().getLocation().toVector().subtract(islandTo.getCenter().toVector()).normalize()
                                .multiply(new Vector(1.2, 0, 1.2));
                        if (DEBUG) {
        ...
        if (islandTo != null && islandFrom == null) {
            // Entering
            if (islandTo.getOwner() != null && (islandTo.isLocked() || plugin.getPlayers()
                    .isBanned(islandTo.getOwner(), player.getUniqueId()))) {
                Util.sendMessage(player, ChatColor.RED + plugin.myLocale(player.getUniqueId()).lockIslandLocked);
            }
            if (islandTo.isSpawn()) {
                ...
                if (islandTo.getOwner() != null && !plugin.myLocale(player.getUniqueId()).lockNowEntering.isEmpty()) {
                    if (islandTo.getIgsFlag(SettingsFlag.ENTER_EXIT_MESSAGES)) {
                        Util.sendEnterExit(player, plugin.myLocale(player.getUniqueId()).lockNowEntering
            ...
            if (islandFrom.isSpawn()) {
                ...
                if (islandFrom.getOwner() != null && !plugin.myLocale(player.getUniqueId()).lockNowLeaving.isEmpty()) {
                    if (islandFrom.getIgsFlag(SettingsFlag.ENTER_EXIT_MESSAGES)) {
                        Util.sendEnterExit(player, plugin.myLocale(player.getUniqueId()).lockNowLeaving
            ...
            if (islandTo.isLocked() || plugin.getPlayers().isBanned(islandTo.getOwner(), e.getPlayer().getUniqueId())) {
                if (!islandTo.getMembers().contains(e.getPlayer().getUniqueId()) && !e.getPlayer().isOp()
                    ...
                    if (e.getPlayer().isInsideVehicle()) {
                        ...
                        //plugin.getLogger().info("DEBUG: " + minMin);
                        if (minMin < 1) {
                            Vector v = e.getPlayer().getLocation().toVector().subtract(islandTo.getCenter().toVector()).normalize()
                                    .multiply(new Vector(1.2, 0, 1.2));
                            if (DEBUG) {
                                plugin.getLogger().info("DEBUG: direction vector = " + v);
                            }
    ...
    public void onEntityDamage(final EntityDamageByEntityEvent e) {
        ...
        if (island != null && island.isSpawn() && e.getDamager().getType().equals(EntityType.CREEPER) && island
                .getIgsFlag(SettingsFlag.CREEPER_PAIN)) {
            return;
        }
        ...
        if (e.getEntity() instanceof ItemFrame || e.getEntityType().toString().endsWith("STAND")) {
            if (island != null && (island.getIgsFlag(SettingsFlag.BREAK_BLOCKS) || island.getMembers()
                    .contains(attacker.getUniqueId()))) {
                return;
            }
            // Else not allowed
        ...
        if (e.getEntity() instanceof Animals || e.getEntity() instanceof IronGolem || e.getEntity() instanceof Snowman
            ...
            if (island != null && (island.getIgsFlag(SettingsFlag.HURT_MOBS) || island.getMembers()
                    .contains(attacker.getUniqueId()))) {
                return;
            }
            if (DEBUG) {
        ...
        boolean pvp = false;
        if ((inNether && island != null && island.getIgsFlag(SettingsFlag.NETHER_PVP) || (!inNether && island != null && island
                .getIgsFlag(SettingsFlag.PVP)))) {
            if (DEBUG) {
                        ...
                        if (Bukkit.getVersion().contains("(MC: 1.8") || Bukkit
                            ...
                            if (e.getPlayer().getItemInHand().getType().equals(Material.WATER_BUCKET)) {
                                ...
                                Util.sendMessage(e.getPlayer(),
                                        ChatColor.RED + plugin.myLocale(e.getPlayer().getUniqueId()).biomeSet.replace("[biome]",
                                                "Nether"));
                            }
                        } else {
                ...
                try {
                    ...
                    if (p.isSplash()) {
                        ...
                        if (island == null) {
                            ...
                            if ((inNether && island.getIgsFlag(SettingsFlag.NETHER_PVP) || (!inNether && island.getIgsFlag(
                                    SettingsFlag.PVP)))) {
                                return;
                            }
                        }
        ...
        if (e.getCause() != null) {
            ...
            switch (e.getCause()) {
                ...
                case FLINT_AND_STEEL:
                    ...
                    if (e.getPlayer() != null && (e.getPlayer().isOp() || VaultHelper.checkPerm(e.getPlayer(),
                            Settings.PERMPREFIX + "mod.bypass"))) {
                        return;
                    }
                    if (!actionAllowed(e.getBlock().getLocation(), SettingsFlag.FIRE)) {
                        if (DEBUG) {
    ...
    public void onEggThrow(PlayerEggThrowEvent e) {
        ...
        if (!inWorld(e.getPlayer()) || e.getPlayer().isOp() || VaultHelper.checkPerm(e.getPlayer(),
                Settings.PERMPREFIX + "mod.bypassprotect")
